...
export function DashboardView({ onNavigate, onOpenForm }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {/* 1. HERO BALANCE CARD (Matching Reference Screenshot) */}
      <div className="hero-balance-card">
        <div className="hero-label">
          <span>Executive Net Worth • Point in Time</span>
          <span style={{ fontSize: '0.75rem', background: 'rgba(255, 255, 255, 0.15)', padding: '0.2rem 0.6rem', borderRadius: '9999px' }}>
            Live Stock Metric
          </span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', paddingTop: '1rem', borderTop: '1px solid rgba(255, 255, 255, 0.15)' }}>
          <div>
            ...
            <div style={{ fontSize: '1.25rem', fontWeight: 800, color: '#34d399', marginTop: '0.2rem' }}>
              +${Number(stock?.total_savings || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
          </div>

          <div>
            <div style={{ fontSize: '0.75rem', opacity: 0.8, display: 'flex', alignItems: 'center', gap: 4 }}>
              <CreditCard size={14} color="#f87171" /> Total Remaining Debt
            </div>
            <div style={{ fontSize: '1.25rem', fontWeight: 800, color: '#f87171', marginTop: '0.2rem' }}>
              -${Number(stock?.total_debt || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
          </div>
        </div>

        {/* Hero Quick Action Tiles (Matching Reference Screenshot) */}
      ...
      <div className="grid-2-col">
        ...
        <div className="glass-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            <div style={{ fontSize: '1rem', fontWeight: 800 }}>Monthly Financial Flow</div>
            <div style={{ fontSize: '0.75rem', background: 'rgba(59, 130, 246, 0.15)', color: 'var(--primary)', padding: '0.25rem 0.6rem', borderRadius: '9999px', fontWeight: 700 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
            <div style={{ background: 'var(--bg-subtle)', padding: '0.875rem', borderRadius: '0.75rem' }}>
              ...
              <div style={{ fontWeight: 800, fontSize: '1.2rem', color: 'var(--success)', marginTop: '0.2rem' }}>
                +${Number(flow?.normalized_income || 0).toFixed(2)}
              </div>
            </div>
            ...
            <div style={{ background: 'var(--bg-subtle)', padding: '0.875rem', borderRadius: '0.75rem' }}>
              ...
              <div style={{ fontWeight: 800, fontSize: '1.2rem', color: 'var(--danger)', marginTop: '0.2rem' }}>
                -${Number(flow?.actual_expense || 0).toFixed(2)}
              </div>
            </div>
          </div>
          ...
          <div style={{ marginTop: '0.875rem', padding: '0.875rem 1rem', background: flow?.net_monthly_flow >= 0 ? 'var(--success-bg)' : 'var(--danger-bg)', borderRadius: '0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ fontSize: '1.25rem', fontWeight: 800, color: flow?.net_monthly_flow >= 0 ? 'var(--success-text)' : 'var(--danger-text)' }}>
              {flow?.net_monthly_flow >= 0 ? '+' : ''}${Number(flow?.net_monthly_flow || 0).toFixed(2)}
            </div>
          </div>
        </div>
          ...
          <div className="grid-3-col" style={{ textAlign: 'center' }}>
            <div style={{ background: 'var(--success-bg)', padding: '0.75rem 0.5rem', borderRadius: '0.75rem' }}>
              ...
              <div style={{ fontWeight: 800, color: 'var(--success-text)', fontSize: '1.05rem', marginTop: '0.2rem' }}>
                +${Number(forecast?.projected_inflows || 0).toFixed(2)}
              </div>
            </div>
            ...
            <div style={{ background: 'var(--danger-bg)', padding: '0.75rem 0.5rem', borderRadius: '0.75rem' }}>
              ...
              <div style={{ fontWeight: 800, color: 'var(--danger-text)', fontSize: '1.05rem', marginTop: '0.2rem' }}>
                -${Number(forecast?.projected_outflows || 0).toFixed(2)}
              </div>
            </div>
            ...
            <div style={{ background: forecast?.projected_net_cash_flow >= 0 ? 'rgba(59, 130, 246, 0.15)' : 'var(--danger-bg)', padding: '0.75rem 0.5rem', borderRadius: '0.75rem' }}>
              ...
              <div style={{ fontWeight: 800, color: 'var(--primary)', fontSize: '1.05rem', marginTop: '0.2rem' }}>
                {forecast?.projected_net_cash_flow >= 0 ? '+' : ''}${Number(forecast?.projected_net_cash_flow || 0).toFixed(2)}
              </div>
            </div>
          </div>
        </div>
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                    ${Number(b.actual_amount).toFixed(2)} of ${Number(b.planned_amount).toFixed(2)} limit
                  </div>
                </div>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  {b.is_over_budget && (
                    <div style={{ fontSize: '0.7rem', color: 'var(--danger)', fontWeight: 700 }}>
                      +${Number(b.overage).toFixed(2)} over
                    </div>
                  )}
                </div>
              </div>
